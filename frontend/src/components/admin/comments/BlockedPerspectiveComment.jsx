...
} from '../../../slices/api_slices/adminApiSlice'; // Import the new API hook
import {
  Table,
  TableBody,
  TableCell,
...
const BlockedPerspectiveCommentsTable = () => {
  ...
  const { data, error, isLoading, isSuccess, refetch } = useGetAllBlockedPerspectiveCommentsQuery({
    pageNum: page + 1,
    limitNum: 10,
  });

  const [unblockPerspectiveCommentFromAdmin, { isLoading: isLoadingUnblock }] = useUnblockPerspectiveCommentFromAdminMutation();
  const [deletePerspectiveCommentFromAdmin, { isLoading: isLoadingDelete }] = useDeletePerspectiveCommentFromAdminMutation();
  ...
  const handleUnblock = async (commentId) => {
    try {
      await unblockPerspectiveCommentFromAdmin({ comment_Id: commentId });
  ...
  const handleDelete = async (commentId) => {
    try {
      await deletePerspectiveCommentFromAdmin({ comment_Id: commentId });
      setErrorGenerated(false);
      setErrorContent('');
      refetch();
    } catch (error) {
    ...
    <Paper sx={{ width: '100%', overflow: 'hidden', backgroundColor:'#d4d5d6', marginBottom:'100px' }}>
      <Box sx={{ padding: 2 }}>
        <Typography variant="h5" align="left" gutterBottom>
          Blocked Perspective Comments
        </Typography>
        <Typography variant="body1" align="left" color='green' gutterBottom>
          Here you can see all the comments of perspectives that are blocked
        </Typography>
      </Box>
      <TableContainer sx={{ maxHeight: 440, minHeight: '300px', backgroundColor:"#ebf1f7" }}>
        <Table stickyHeader aria-label="sticky table">
          ...
          {commentsData.map((comment) => (
              <TableRow hover role="checkbox" tabIndex={-1} key={comment.comment_id} sx={{
                '&:nth-of-type(odd)': {
...
export default BlockedPerspectiveCommentsTable;
