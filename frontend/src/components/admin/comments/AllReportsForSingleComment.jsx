...
import { useGetAllReportsForSingleCommentFromAdminQuery, useDeleteReportOnCommentMutation } from '../../../slices/api_slices/adminApiSlice'; // Adjust path as per your API setup
import relativeTime from '../../../utils/relativeTime';

const ReportsModal = ({ commentId, open, onClose }) => {
    console.log(commentId)
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);

  const { data, isLoading, isError, isSuccess, refetch } = useGetAllReportsForSingleCommentFromAdminQuery({
    page: page + 1, // API pagination starts from 1
    limit: rowsPerPage,
    commentId: commentId,
  });

  const [deleteReport] = useDeleteReportOnCommentMutation();

  useEffect(() => {
    refetch();
  }, [page, rowsPerPage, commentId, refetch]);
  ...
  const handleDeleteReport = async (reportId) => {
    try {
      await deleteReport({ reportComment_Id: reportId });
      refetch();
    } catch (error) {
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box sx={{
        ...
        <Typography variant="h5" mb={2}>Reports for Comment </Typography>
                  ...
                  {data?.reports.map((report) => (
                    <TableRow key={report._id}>
                      ...
                      <TableCell align="center">
                        <IconButton onClick={() => handleDeleteReport(report._id)} color="secondary">
                          <DeleteIcon />
                        </IconButton>
                      </TableCell>
                    </TableRow>
