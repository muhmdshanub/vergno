...
const SuggestionsTopicsCard = ({topicData, onSuggestionClear }) => {
  ...
  const [errorDialogOpen, setErrorDialogOpen] = useState(false);
   ...
   const [followTopic, { isLoading: followLoading, isSuccess: followSuccess }] = useFollowTopicMutation();
  ...
  const handleFollow = async () => {
    try {
      await followTopic({ topicId: topicData._id }).unwrap();

      setToggleSwitch("unfollow");
    } catch (error) {
      
      setErrorFlag(JSON.stringify(error?.data?.message || error?.message) || 'Error following topic');
...
const handleCloseErrorDialog = () => {
  ...
  return (
    <GlassmorphicCard elevation={6} >
      <IconButton aria-label="close" onClick={() => onSuggestionClear(topicData?._id)} sx={{ position: "absolute", top: "10px", right: "10px", width: "fit-content", color:'#000000' }}>
        <Close />
      </IconButton>
      <CardContent>
        <Box display="flex" flexDirection="column" alignItems="center" justifyItems="center">
          ...
          {toggleSwitch === "follow" ? (
            <GlassmorphicButton variant="contained" onClick={handleFollow}>
              {followLoading ? <GradientCircularProgress /> : 'Follow'}
            </GlassmorphicButton>
          ) : (
...
SuggestionsTopicsCard.propTypes = {
  topicData: PropTypes.object.isRequired,
  onClose: PropTypes.func.isRequired,
};

export default SuggestionsTopicsCard;
