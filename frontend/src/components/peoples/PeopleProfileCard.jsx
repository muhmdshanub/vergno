    ...
    <GlassmorphicCard elevation={6}>
      ...
      <CardContent >
        <Box display="flex" flexDirection="column" alignItems="center" justifyItems="center" >
          ...
          <Box  display="flex" flexDirection="column" alignItems="center" justifyItems="center" sx={{backgroundColor: userData.followingTopics.length > 0 ? "#000000" : 'transparent',color:"#ffffff", width:"90%", paddingTop:"8px", paddingBottom:"8px", borderRadius:"5px", marginBottom:"2rem", height:'4rem', }} >
            
            {
              ...
              userData.followingTopics.map((topic) =>(
                ...
                <Tooltip title={topic.topicDetails.name} key={topic._id}>
                  <Typography variant="body2"  sx={{cursor : 'pointer', mb: '3px',mt:0, width: '100%', height: '1rem',  display:"flex",justifyContent:"center", color:'#ffffff',}} onClick={() => navigate(`/topics/${topic.topicDetails._id}`)}>
